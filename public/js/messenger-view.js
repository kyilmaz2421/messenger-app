...
const getMessages = (elemMain,lim ,skip,location=0)=>{
    ...
    fetch("/chatrooms/conversation/"+chatroomID+"?limit="+lim+"&skip="+skip,{
        method: 'GET'
    }).then((res)=>{ 
        res.json().then((conversation)=>{
            if(conversation.error){
                document.querySelector(".chat__messages").innerHTML='<h3 style="padding-left:36%;padding-top:10%;color:rgb(168, 164, 164);"> '+conversation.error+'</h3>'
            }else if(location == -1 || renderChatroomName(chatroomID)){
                if(conversation.data.length==0){
                    if(location ==0) document.querySelector(".chat__messages").innerHTML='<h3 style="padding-left:36%;padding-top:10%;color:rgb(168, 164, 164);">No conversation yet</h3>'
                    else if(document.querySelector(".chat__messages").children[0].innerHTML!='<p style="padding-left:36%;color:rgb(168, 164, 164);">Conversation up to date</p>'){
                        const message = document.createElement("div")
                        message.innerHTML= '<p style="padding-left:36%;color:rgb(168, 164, 164);">Conversation up to date</p>'
                    ...
                    if(elemMain.innerHTML.search('id="newGroup-text"')>0){
                        elemMain.children[2].innerHTML = elemMain.children[2].innerHTML.replace('id="newGroup-text"',"")     
                ...
                document.querySelector(".chat__messages").innerHTML='<h3 style="padding-left:36%;padding-top:10%;color:rgb(168, 164, 164);"> Server Error: cannot retreive conversation</h3>'
            }
        })
    })
    .catch((e) => {
...
clearConvPage = (isDeleted)=>{
    ...
    if (isDeleted){
        document.querySelector(".chat__messages").innerHTML='<h3 style="padding-left:36%;padding-top:10%;color:rgb(168, 164, 164);">No conversation Selected</h3>'
...
const renderMessage = (message,location=0) =>{
    ...
    if(message.user===username) span.setAttribute("class","user-message")
    else span.setAttribute("class","other-message")
    ...
    if(message.user!=="Admin"){
        ...
        metadata.innerHTML=  '<span class="message__name">'+message.user+'</span> <span class="message__meta">'+time+'</span>'
        ...
        elem.innerHTML = '<p style="padding-left:30%;color:rgb(168, 164, 164);"> <span class="message__name">'+message.user+':</span> <span class="message__meta">'+message.text +" &#9679; "+time+'</span> </p>'
    }
