...
$messageForm.addEventListener("submit", (ev) => {
  ...
  const text = ev.target.elements.message.value,
    chatroomID = chatroomIDElem.className.slice(7);

  socket.emit("sendMessage", { text, username, chatroomID }, (message) => {
    // run upon event emitted is acknowledged by reciever
...
//RECIEVE MESSAGE
socket.on("message", ({ message, chatroomID, username: user }) => {
  //general for all messages data recieved
  updateChatbox(document.getElementById(chatroomID), message, true);
...
socket.on("newNotification", ({ id, data, username: user }) => {
  ...
  if (data.delete) {
    ...
    if (data.oldData.oldMembers) {
      //members change
      if (
        data.oldData.oldMembers.includes(username) &&
        !data.chatroom.members.includes(username)
      ) {
        //delete or leave
