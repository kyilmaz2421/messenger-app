...
const renderUserTag = (user, buttonType) => {
  data =
    '<button class="ui ' +
    buttonType +
    ' basic button" style="margin-top: 10px;"> <span class="new-user">' +
    user +
    '</span> <i style="padding-left: 3px;" class="close icon"> </i></button>';
  ...
  elem.addEventListener("click", function (e) {
    ...
    if (this.className == username) {
      toggleErrorMessages(
        ...
        "NOTE: You have selected yourself to leave the group",
        "info"
      );
    }
    if (users.size < 2) {
      toggleErrorMessages(
        "edit-convo-error-messages",
        "NOTE: If a group with less than two members is submitted (including yourself) the group will be deleted!",
        "info"
      );
    }
...
const renderEditPage = (chatroomid) => {
  ...
  document.querySelector(".edit__conversation h5").innerText =
    "current members shown below in red (remove members by clicking)";
  document.querySelector("#submit-edit-conversation").innerText =
    "Submit Changes";
      ...
      fetch(url, {
        ...
        .then((res) => {
          res.json().then((data) => {
            data.method = method;
            if (data.error)
              toggleErrorMessages("edit-convo-error-messages", data.error);
            else if (method == "POST") {
