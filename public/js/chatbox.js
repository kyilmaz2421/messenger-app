...
const addChatbox = (chatroom,location,newGroup=false)=>{
    ...
    if (newGroup){
        elemData.innerHTML = '<div style="font-size:medium;">'+chatroom.name+' </div> <p> <span id="newGroup-text" style="font-size:90%;"> You have been added to this chatroom </span> </p>' 
    }else{
        const content = chatroom.conversation.data[0]
        elemData.innerHTML = '<div style="font-size:medium;">'+chatroom.name+' </div> <p> <span style="font-size:90%;">'+content.user+': '+content.text+' <span style="font-size:6px;">&#9679;</span> '+date+' </span> </p>' 
    }
...
const toggleNotificationIcon = (chatroomID,isRemove)=>{
    ...
    if(isRemove && icon.style.display==="block"){ //remove --> remove when it is actually there
        elem.style.fontWeight="normal"
        icon.style.display="none"
       
    }else if(!isRemove && icon.style.display==="none"){ //add  --> add if it isn't present
...
const updateChatbox = (elem,data,isMessage=false)=>{
    if(data.oldData){ //when we have PATCH requests that don't populate conversation
        ...
        if (data.oldData.oldName) buffer = buffer.replace(data.oldData.oldName,data.chatroom.name)
        if (data.oldData.oldDate) buffer = buffer.replace(data.oldData.oldDate,date)
        ...
        elem.children[2].innerHTML = buffer.slice(0,index) + ' </div> <p> <span style="font-size:90%;">'+data.user+': '+data.text+' <span style="font-size:6px;">&#9679;</span> '+date+' </span> </p>' 
    }else{ //when we have the conversation populated in the chatroom (GET request)
        ...
        //}else
        elem.children[2].innerHTML = '<div style="font-size:medium;">'+data.chatroom.name+' </div> <p> <span style="font-size:90%;">'+content.user+': '+content.text+' <span style="font-size:6px;">&#9679;</span> '+date+' </span> </p>' 
        ...
        res.json().then((data)=>{
            if(data.error){
                ...
                data.chatrooms.forEach(chatroom=>{
                    ...
                    if(data.notifications.includes(chatroom._id)) toggleNotificationIcon(chatroom._id,false)
                })
                if(!onLoad){
