...
const isEditGroupRequestError = async (
  patchQuery,
  newMembers,
  isPatchRequest = true
) => {
  const dbQuery = isPatchRequest
    ? await Chatroom.findOne(patchQuery)
    : await Chatroom.findOne({ members: newMembers });
  const queryFound = dbQuery ? true : false;
  if (newMembers) {
    // in the 'leave' and 'name' PATCH requests we dont need to verify members
...
const updatePatchReqMetaData = (chatroom, newMembers, newName = undefined) => {
  ...
  } else if (chatroom.members.join(", ") === chatroom.name) {
    // old members == name
    oldData.oldName = chatroom.name;
...
const sliceAndPopulate = async (chatrooms, left, right) => {
  ...
  for (i = 0; i < slicedChatrooms.length; i++) {
    ...
    slicedChatrooms[i].conversation.data = slicedChatrooms[
      i
    ].conversation.data.slice(-1);
  }
