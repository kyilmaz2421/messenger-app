...
router.post("/chatrooms", auth, async (req, res) => {
  try {
    ...
    if (!newMembers.includes(req.user.username)) {
      return res
        .status(404)
        .send({ error: "User must be in the group they create" });
    }
...
router.get("/chatrooms", auth, async (req, res) => {
  try {
    ...
    if (skip == chatrooms.length && chatrooms.length > 0) {
      ...
      const slicedChatrooms = await controller.sliceAndPopulate(
        chatrooms,
        0,
        lim
      );
...
router.get("/chatrooms/conversation/:chatroomid", auth, async (req, res) => {
  try {
    ...
    if (skip == 0) {
      if (conversation.data.length >= lim) {
        // lst
        conversation.data = conversation.data.slice(
          conversation.data.length - lim,
          conversation.data.length
        );
      } // alternativeley we send the whole thing
    } else if (skip >= conversation.data.length) {
      // skip >0 (therefore not the lst query --> looking up chat history)
      conversation.data = [];
    } else if (conversation.data.length >= lim + skip) {
      conversation.data = conversation.data.slice(
        conversation.data.length - (lim + skip),
        conversation.data.length - skip
      );
    } else {
      // from skip to the rest
      conversation.data = conversation.data.slice(
        0,
        conversation.data.length - skip
      );
...
//PATCH ROUTES
router.patch("/chatrooms/:chatroomid/edit", auth, async (req, res) => {
  ...
  try {
    ...
    const oldData = controller.updatePatchReqMetaData(
      validation.chatroom,
      newMembers
    );
    ...
    if (newMembers.length < 2) {
      return res
        .status(200)
        .send(await controller.deleteChatroom(validation.chatroom, oldData));
    } else if (!newMembers.includes(req.user.username)) {
      await deleteNotificationDocument(
        req.user.username,
...
router.patch("/chatrooms/:chatroomid/name", auth, async (req, res) => {
  //join an existing chat
  try {
    const validation = await controller.isEditGroupRequestError(
      controller.chatroomQuery(req.params.chatroomid, req.user),
...
router.patch("/chatrooms/:chatroomid/leave", auth, async (req, res) => {
  //join an existing chat
  try {
    const validation = await controller.isEditGroupRequestError(
      controller.chatroomQuery(req.params.chatroomid, req.user),
    ...
    const newMembers = validation.chatroom.members
      .filter((user) => user != req.body.username)
    ...
    if (newMembers.length < 2) {
      return res.send(
        await controller.deleteChatroom(validation.chatroom, oldData)
      );
    } else {
      validation.chatroom.members = newMembers;
