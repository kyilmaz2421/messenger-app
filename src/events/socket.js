...
const {getUser, goOffline, goOnline, updateSockets} = require("../controller/user");
...
const socketConfig = async (socket, io) => {
    ...
    socket.on('sendMessage',async ({text, username, chatroomID}, callback)=>{ // need to verify roomID and username auth
        cache.getChatroom(chatroomID).then( async (members)=>{
            await generateNotifications(chatroomID, members.split(","), eventCreatorUsername = username);
    ...
    socket.on("sendNotification",async ({roomID, data, username})=>{ 
        ...
        if(!data.delete){ // send out notification regarding changes if the change isn't the group being deleted
            generateNotifications(roomID, data.chatroom.members, eventCreatorUsername = username);
        }else{
